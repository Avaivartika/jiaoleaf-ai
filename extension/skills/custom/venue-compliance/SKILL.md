...
Use the `mcp__jiaoleaf-interactive__ask_user` MCP tool to collect venue details. Ask structured questions with clickable options where appropriate:
...
- If guidelines are not found, use `mcp__jiaoleaf-interactive__ask_user` to request a direct URL
...
- **Guidelines not found**: Use `mcp__jiaoleaf-interactive__ask_user` to request a direct URL to the guidelines page
- **No .tex files in context**: Report an error — compliance checking requires manuscript source
- **Unknown venue**: Use `mcp__jiaoleaf-interactive__ask_user` to ask for more details or a guidelines URL
