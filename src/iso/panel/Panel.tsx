...
const CODEX_DEFAULT_MODEL_VALUE = 'gpt-5.4';
const CODEX_DEFAULT_MODEL_LABEL = 'GPT-5.4';
const INTERRUPTED_BY_USER_MARKER = 'INTERRUPTED BY USER';
const DEBUG_DIFF = false;
const DEFAULT_ENABLED_SKILL_NAMES = [
  'citation-management',
  'venue-compliance',
  'paper-reviewer',
  'literature-review',
...
function normalizeEditorThemeName(name: string) {
  return name.toLowerCase().replace(/^cm-s-/, '').replace(/[\s-]+/g, '_');
}

function estimateRgbLightness(value: string) {
...
function detectOverleafLightMode() {
  const themedElement = document.querySelector('[class*="cm-s-"], .cm-editor');
  const themeClasses =
    themedElement instanceof HTMLElement
      ? Array.from(themedElement.classList)
        .filter((className) => className.startsWith('cm-s-'))
        .map(normalizeEditorThemeName)
      : [];

  if (themeClasses.some((theme) => DARK_EDITOR_THEMES.has(theme))) return false;
  if (themeClasses.some((theme) => LIGHT_EDITOR_THEMES.has(theme))) return true;

  const editor = document.querySelector('.cm-content, .cm-editor');
  if (editor instanceof HTMLElement) {
    const { backgroundColor, color } = window.getComputedStyle(editor);
    const backgroundLightness = estimateRgbLightness(backgroundColor);
    const textLightness = estimateRgbLightness(color);
    if (backgroundLightness !== null && textLightness !== null) {
...
const CODEX_FALLBACK_MODELS: RuntimeModel[] = [
  {
    value: CODEX_DEFAULT_MODEL_VALUE,
    displayName: CODEX_DEFAULT_MODEL_LABEL,
...
function getCodexThinkingModes(model: RuntimeModel | null): ThinkingMode[] {
  const supportedModes = new Set(
    (model?.supportedReasoningEfforts ?? []).map((entry) =>
      getThinkingModeIdForCodexEffort(String(entry.reasoningEffort ?? ''))
    )
  );
  const modes = FALLBACK_THINKING_MODES.map((mode) => ({
    ...mode,
...
const Panel = () => {
  ...
  const [isLightMode, setIsLightMode] = useState(() => detectOverleafLightMode());
  ...
  useEffect(() => {
    const syncFromOverleaf = () => {
      if (!themeOverriddenRef.current) {
        setIsLightMode(detectOverleafLightMode());
      }
    };

    const observer = new MutationObserver(syncFromOverleaf);
  ...
  const [settingsTab, setSettingsTab] = useState<
    'connection' | 'tools' | 'skills' | 'customization' | 'safety'
  >('connection');
  ...
  const [skillResults, setSkillResults] = useState<PanelSkill[]>([]);
  const [allSkills, setAllSkills] = useState<PanelSkill[]>([]);
  const [skillIndex, setSkillIndex] = useState(0);
  const [runtimeMenuOpen, setRuntimeMenuOpen] = useState<
    'model' | 'thinking' | null
  >(null);
  ...
  const normalizeCustomSkills = (skills: Options['customSkills']): PanelSkill[] =>
    (skills ?? [])
      .map((skill) => {
        const name = (skill.name ?? '').trim();
        const content = (skill.content ?? '').trim();
        if (!name || !content) return null;
        const id = skill.id || `custom:${name.toLowerCase().replace(/[^a-z0-9._-]+/g, '-')}`;
        const normalized: PanelSkill = {
          id,
          name,
          description: (skill.description ?? 'Custom skill').trim() || 'Custom skill',
          tags: Array.isArray(skill.tags) ? skill.tags : [],
          source: 'custom-user',
          path: `custom:${id}`,
          autoContext: Array.isArray(skill.autoContext) ? skill.autoContext : [],
          customContent: content,
        };
        return normalized;
      })
      .filter((skill): skill is PanelSkill => Boolean(skill));

  const getDefaultEnabledSkillIds = (skills: PanelSkill[]) => {
    const byName = new Map(skills.map((skill) => [skill.name.toLowerCase(), skill.id]));
    return DEFAULT_ENABLED_SKILL_NAMES.map((name) => byName.get(name)).filter(
      (id): id is string => Boolean(id)
    );
  };

  const getEffectiveEnabledSkillIds = (skills = allSkills) => {
    const configured = settings?.enabledSkillIds ?? [];
    return configured.length > 0 ? configured : getDefaultEnabledSkillIds(skills);
  };

  const enabledSkills = useMemo(() => {
    const enabledIds = new Set(getEffectiveEnabledSkillIds(allSkills));
    return allSkills.filter((skill) => enabledIds.has(skill.id));
  }, [allSkills, settings?.enabledSkillIds]);
  ...
  useEffect(() => {
    ...
    const loadSkills = async () => {
      try {
        ...
        setAllSkills([
          ...manifest.skills,
          ...normalizeCustomSkills(settings?.customSkills),
        ]);
      } catch {
        if (!cancelled) setAllSkills(normalizeCustomSkills(settings?.customSkills));
      }
    };
  ...
  useEffect(() => {
    if (!runtimeMenuOpen && !skillMenuOpen) return;
  ...
  useEffect(() => {
    ...
    const loadRuntime = async () => {
      ...
      if (chatProvider === 'pi') {
        ...
        setRuntimeModels(CODEX_FALLBACK_MODELS);
        setThinkingModes(
          getCodexThinkingModes(CODEX_FALLBACK_MODELS[0]).map((mode) => ({
            ...mode,
            maxThinkingTokens: null,
          }))
        );
        setCurrentThinkingMode(
          getThinkingModeIdForCodexEffort(
            ...
            CODEX_FALLBACK_MODELS[0]?.defaultReasoningEffort
          )
        );
        setCurrentThinkingTokens(null);
        setCurrentModel(options.codexModel ?? CODEX_DEFAULT_MODEL_VALUE);
      } else {
      ...
      try {
        ...
        if (chatProvider === 'codex') {
          ...
          if (cached && cacheAge < CACHE_TTL) {
            ...
            if (cancelled) return;
            const discoveredModels = (metadata.models ?? []).filter(
              (model: RuntimeModel) => !model.value.includes('gpt-5.1')
            );
            models = discoveredModels.length > 0 ? discoveredModels : CODEX_FALLBACK_MODELS;
          }

          const modelOptions = models.length > 0 ? models : CODEX_FALLBACK_MODELS;
          setRuntimeModels(modelOptions);

          // Determine model selection
          const resolvedModel = metadata
            ? metadata.currentModel ??
            options.codexModel ??
            modelOptions.find((model: RuntimeModel) => model.isDefault)?.value ??
          ...
          const selectedModel =
            (resolvedModel
              ...
              : undefined) ??
            modelOptions.find((model: RuntimeModel) => model.isDefault) ??
            modelOptions[0] ??
            null;
          const nextThinkingModes = getCodexThinkingModes(selectedModel);
          setThinkingModes(nextThinkingModes);
  ...
  const updateSkillState = async () => {
    ...
    try {
      const searchableSkills =
        enabledSkills.length > 0 ? enabledSkills : allSkills;
      const results = searchSkills(searchableSkills, match.query);
      if (results.length === 0) {
        setSkillOpen(false);
        setSkillResults([]);
        skillRangeRef.current = null;
  ...
  const insertSkill = (skill: PanelSkill) => {
    const editor = editorRef.current;
    if (!editor) return;
    const selection = window.getSelection();
    if (!selection) return;
  ...
  const processSkillDirectives = async (
    ...
    // Load skills and find matching enabled skills.
    try {
      const options = settings ?? (await getOptions());
      const manifestSkills: PanelSkill[] =
        allSkills.length > 0
          ? allSkills
          : [
            ...((await loadSkillsManifest()).skills as PanelSkill[]),
            ...normalizeCustomSkills(options.customSkills),
          ];
      const enabledIds = new Set(
        (options.enabledSkillIds?.length ?? 0) > 0
          ? options.enabledSkillIds
          : getDefaultEnabledSkillIds(manifestSkills)
      );
      const availableSkills = manifestSkills.filter((skill) =>
        enabledIds.has(skill.id)
      );
      ...
      for (const name of directiveNames) {
        ...
        if (skill) {
          const markdown = skill.customContent ?? (await loadSkillMarkdown(skill));
  ...
  const findRuntimeModel = (token: KnownModelToken) => {
    return getRuntimeModelOptions().find(
      (model) =>
  ...
  const getDefaultModelValue = () => {
    if (chatProvider === 'codex') {
      return settings?.codexModel ?? CODEX_DEFAULT_MODEL_VALUE;
    }
    if (chatProvider === 'pi') {
      return settings?.piModel ?? null;
    }
    return settings?.claudeModel ?? DEFAULT_MODEL_VALUE;
  };

  const getRuntimeModelOptions = () => {
    if (chatProvider === 'codex') {
      return runtimeModels.length > 0 ? runtimeModels : CODEX_FALLBACK_MODELS;
    }
    if (chatProvider === 'claude') {
      return runtimeModels.length > 0 ? runtimeModels : CLAUDE_FALLBACK_MODELS;
    }
  ...
  const getOrderedRuntimeModels = () => {
    if (chatProvider === 'claude') {
      ...
      return ordered.length > 0 ? ordered : getRuntimeModelOptions();
    }
    return getRuntimeModelOptions();
  };
  ...
  const getRuntimeModelLabel = (model: RuntimeModel) => {
    ...
    const displayName = model.displayName ?? model.value;
  ...
  const isRuntimeModelSelected = (model: RuntimeModel) => {
    const resolved = currentModel ?? getDefaultModelValue();
  ...
  const getSelectedModelLabel = () => {
    const resolvedModel = currentModel ?? getDefaultModelValue();
    if (chatProvider === 'pi' && !resolvedModel) {
      return 'No model';
    }
    const resolvedToken = getKnownModelToken(resolvedModel);
    if (chatProvider === 'claude' && resolvedToken && resolvedToken in MODEL_DISPLAY) {
      return MODEL_DISPLAY[resolvedToken].label;
    }
    const modelOptions = getRuntimeModelOptions();
    const match =
      modelOptions.find((model) => model.value === resolvedModel) ??
      (chatProvider === 'claude' ? modelOptions.find(
        (model) =>
    ...
    const fallbackLabel = resolvedModel ?? DEFAULT_MODEL_LABEL;
    return fallbackLabel.replace(/\bgpt\b/gi, 'GPT').replace(
      /\bcodex\b/gi,
      'Codex'
    );
  };
  ...
  const onSelectModel = async (model: string) => {
    ...
    if (chatProvider === 'codex') {
      const selectedModel =
        ...
        CODEX_FALLBACK_MODELS.find((entry) => entry.value === model) ??
      ...
      await persistRuntimeOptions({
        ...
        codexReasoningEffort:
          getCodexEffortForThinkingMode(nextMode, selectedModel) ??
          selectedModel?.defaultReasoningEffort,
      });
      setContextUsage(null);
      void refreshContextUsage();
  ...
  const onSelectThinkingMode = async (modeId: string) => {
    ...
    if (chatProvider === 'codex') {
      const selectedModel =
        (currentModel
          ? runtimeModels.find((entry) => entry.value === currentModel) ??
          CODEX_FALLBACK_MODELS.find((entry) => entry.value === currentModel)
          : null) ??
        runtimeModels.find((entry) => entry.isDefault) ??
        runtimeModels[0] ??
        CODEX_FALLBACK_MODELS[0] ??
      ...
      await persistRuntimeOptions({
        codexReasoningEffort:
          getCodexEffortForThinkingMode(modeId, selectedModel) ??
          selectedModel?.defaultReasoningEffort,
      });
      return;
    }
  ...
  const sendMessage = async (
    ...
    try {
      ...
      const codexModelCandidate =
        provider === 'codex'
          ? currentModel ?? options.codexModel ?? CODEX_DEFAULT_MODEL_VALUE
          : null;
      const codexRuntimeModel =
        provider === 'codex'
          ? (codexModelCandidate
            ? codexRuntimeOptions.find(
              (entry) => entry.value === codexModelCandidate
            )
            : null) ??
          codexRuntimeOptions.find((entry) => entry.isDefault) ??
          codexRuntimeOptions.find(
            (entry) => entry.supportedReasoningEfforts !== undefined
          ) ??
          codexRuntimeOptions[0] ??
          null
          : null;
      const codexModel =
        provider === 'codex' &&
  ...
  const onSaveSettings = async () => {
    ...
    try {
      ...
      // Sync Pi-only trust preference only while the Pi provider is active.
      if (chatProvider === 'pi' && settings.skillTrustMode) {
        updatePiRuntimePreferences(settings, { skillTrustMode: settings.skillTrustMode }).then(
          ...
          () => { /* next Pi health check will retry */ },
        );
      }
    } catch (error) {
      if (
  ...
  const updateCustomSkillDraft = (next: Partial<CustomSkillDraft>) => {
    setCustomSkillDraft((prev) => ({ ...prev, ...next }));
  };

  const toggleSkillEnabled = (skillId: string, enabled: boolean) => {
    if (!settings) return;
    const current = new Set(getEffectiveEnabledSkillIds());
    if (enabled) {
      current.add(skillId);
    } else {
      current.delete(skillId);
    }
    updateSettings({ enabledSkillIds: Array.from(current) });
  };

  const enableAllSkills = () => {
    updateSettings({ enabledSkillIds: allSkills.map((skill) => skill.id) });
  };

  const resetDefaultSkills = () => {
    updateSettings({ enabledSkillIds: getDefaultEnabledSkillIds(allSkills) });
  };

  const addCustomSkill = () => {
    if (!settings) return;
    const name = customSkillDraft.name.trim();
    const content = customSkillDraft.content.trim();
    if (!name || !content) {
      setSettingsMessage('Custom skill needs a name and instructions.');
      return;
    }
    const id = `custom:${Date.now().toString(36)}-${name
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, '-')
      .replace(/^-|-$/g, '')}`;
    const tags = customSkillDraft.tags
      .split(',')
      .map((tag) => tag.trim())
      .filter(Boolean);
    const autoContext = customSkillDraft.autoContext
      .split(',')
    ...
    const customSkills = [
      ...(settings.customSkills ?? []),
      {
        id,
        name,
        description: customSkillDraft.description.trim() || 'Custom skill',
        tags,
        autoContext,
        content,
      },
    ...
    updateSettings({
      customSkills,
      enabledSkillIds: Array.from(new Set([...getEffectiveEnabledSkillIds(), id])),
    });
    setCustomSkillDraft({
      name: '',
      description: '',
    ...
    setSettingsMessage('Custom skill added. Save settings to persist it.');
  };

  const removeCustomSkill = (skillId: string) => {
    if (!settings) return;
    updateSettings({
      customSkills: (settings.customSkills ?? []).filter(
        (skill) => skill.id !== skillId
      ),
      enabledSkillIds: getEffectiveEnabledSkillIds().filter((id) => id !== skillId),
    });
  };
    ...
    if (evicted.length > 0) {
      ...
      for (const evictedId of evicted) {
        ...
        try {
          ...
          // Best-effort cleanup; the local UI state is still safe to remove.
        }
      }
    }
                ...
                <div class="jiaoleaf-toolbar-actions">
                  ...
                  >
                    <button
                      ...
                      aria-label="Open enabled skills"
                      data-tooltip="Skills"
                      onClick={() => {
                        setRuntimeMenuOpen(null);
                      ...
                      <div class="jiaoleaf-settings__skill-fields">
                        ...
                        <label>
                          ...
                          <input
                            ...
                            onInput={(event) =>
                              updateCustomSkillDraft({
                                autoContext: (event.target as HTMLInputElement).value,
                              })
                            }
                            placeholder="*.tex, *.bib"
                          />
