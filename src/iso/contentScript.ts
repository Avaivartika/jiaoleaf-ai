...
let sidecarHost: HTMLElement | null = null;
let sidecarResizeObserver: ResizeObserver | null = null;
...
function isSjtuOverleafHost() {
  return (
    window.location.hostname === 'latex.sjtu.edu.cn' ||
    window.location.hostname === 'latex-en.sjtu.edu.cn'
  );
}
...
function mountLayout(): HTMLElement {
  ...
  if (isSjtuOverleafHost()) {
    const host = findLayoutHost();
    const layout = document.createElement('div');
    layout.id = LAYOUT_ID;
    layout.className = `${LAYOUT_ID} ${LAYOUT_SIDECAR_CLASS}`;
    ...
    if (host) {
      ...
      sidecarResizeObserver = new ResizeObserver(updateSidecarWidth);
      sidecarResizeObserver.observe(layout);
      window.requestAnimationFrame(updateSidecarWidth);
    }
    return layout;
  }

  const host = findLayoutHost();
  const useOverlay = !host?.parentElement;
  const layout = document.createElement('div');
  layout.id = LAYOUT_ID;
  layout.className = useOverlay ? `${LAYOUT_ID} ${LAYOUT_OVERLAY_CLASS}` : LAYOUT_ID;

  const main = document.createElement('div');
  main.className = LAYOUT_MAIN_CLASS;
  layout.appendChild(main);

  if (!useOverlay && host && host.parentElement) {
...
function unmountLayout() {
  ...
  if (layout.classList.contains(LAYOUT_SIDECAR_CLASS)) {
    sidecarResizeObserver?.disconnect();
    sidecarResizeObserver = null;
    sidecarHost?.classList.remove(SIDECAR_HOST_CLASS);
    sidecarHost?.style.removeProperty(SIDECAR_WIDTH_VAR);
    sidecarHost = null;
    layout.remove();
    return;
  }
