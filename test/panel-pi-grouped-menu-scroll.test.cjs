...
test('PI grouped model menu uses flyout submenu positioned to the left', () => {
  ...
  assert.match(
    contents,
    /\.jiaoleaf-runtime__menu--grouped\s*\{[^}]*overflow:\s*visible/s,
    'Grouped PI menu must not clip flyout submenu (overflow: visible)'
  );

  assert.match(
    contents,
    /\.jiaoleaf-runtime__group-models\s*\{[^}]*position:\s*absolute/s,
    'Model submenu should be absolutely positioned for flyout'
  );

  assert.match(
    contents,
    /\.jiaoleaf-runtime__group:hover\s*>\s*\.jiaoleaf-runtime__group-models\s*\{[^}]*display:\s*grid/s,
