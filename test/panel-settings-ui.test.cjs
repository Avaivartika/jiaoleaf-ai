...
test('Panel includes settings button and tabs', () => {
  ...
  assert.match(contents, /customSkills/);
...
test('Landing page uses large icon and product tagline', () => {
  const panelPath = path.join(
    __dirname,
    '..',
    'src',
  ...
  assert.match(contents, /PRODUCT_LOGO_256/);
  assert.match(contents, /PRODUCT_NAME/);
  assert.match(contents, /PRODUCT_TAGLINE/);
});
