...
test('Panel includes Overleaf-integrated visual tokens', () => {
  const cssPath = path.join(__dirname, '..', 'src', 'iso', 'panel', 'jiaoleaf-toolbar-components.css');
  const css = fs.readFileSync(cssPath, 'utf8');

  assert.match(css, /OVERLEAF-INTEGRATED THEME/);
  assert.match(css, /--jiaoleaf-panel-bg:\s*#f7f8fa/);
  assert.match(css, /--jiaoleaf-panel-accent:\s*#138a43/);
  assert.match(css, /background:\s*#17212b/);
  assert.match(css, /background:\s*#2d3a48/);
  assert.match(css, /\.jiaoleaf-toolbar-strip/);
  assert.match(css, /\.jiaoleaf-skill-menu__list/);
  assert.match(css, /\.jiaoleaf-panel__name/);
...
test('Panel exposes skills through a menu and settings page', () => {
  ...
  assert.match(contents, /jiaoleaf-skill-menu/);
  ...
  assert.doesNotMatch(contents, /jiaoleaf-skill-rail/);
