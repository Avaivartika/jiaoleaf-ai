...
test('Panel checks latest main commit and renders a dismissible update banner', () => {
  ...
  const brandPath = path.join(__dirname, '..', 'src', 'iso', 'panel', 'brand.ts');
  const contents = `${fs.readFileSync(panelPath, 'utf8')}\n${fs.readFileSync(brandPath, 'utf8')}`;

  assert.match(
    contents,
    /https:\/\/api\.github\.com\/repos\/Avaivartika\/jiaoleaf-ai\/commits\/main/
  );
  assert.match(contents, /jiaoleaf-panel__update-banner/);
