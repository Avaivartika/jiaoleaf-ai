...
test('generateSkillsManifest creates valid manifest', async () => {
  ...
  // - science: focused domain skills inspired by broad scientific assistant workflows
  assert.strictEqual(manifest.skills.length, 15, `Expected 15 skills, got ${manifest.skills.length}`);
  ...
  for (const skillName of scienceSkills) {
    const skill = manifest.skills.find((s) => s.name === skillName);
    assert.ok(skill, `Should include ${skillName} skill`);
    assert.strictEqual(skill.source, 'science', `${skillName} should be from science source`);
  }
