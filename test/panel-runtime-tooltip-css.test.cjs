...
test('Context usage tooltip uses data-tooltip hover styles', () => {
  ...
  assert.match(contents, /\.jiaoleaf-runtime__usage::after\s*\{[^}]*content:\s*attr\(data-tooltip\)/s);
  assert.match(contents, /\.jiaoleaf-runtime__usage:hover::after\s*\{[^}]*opacity:\s*1;/s);
});
