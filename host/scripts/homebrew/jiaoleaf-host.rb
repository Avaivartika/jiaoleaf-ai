class JiaoLeafHost < Formula
  desc "JiaoLeaf companion host for Chrome native messaging (macOS)"
  homepage "https://github.com/Avaivartika/jiaoleaf-ai"
  license "MIT"
  ...
  #   url "https://github.com/Avaivartika/jiaoleaf-ai/releases/download/v0.1.8/jiaoleaf-host-macos-node-bundle.tar.gz"
  ...
  def install
    bundle_root = (buildpath/"jiaoleaf-host-bundle").directory? ? (buildpath/"jiaoleaf-host-bundle") : buildpath
    ...
    SH
    chmod 0755, bin/"jiaoleaf-host"

    (bin/"jiaoleaf-host-install-manifest").write <<~SH
      #!/usr/bin/env bash
      set -euo pipefail
    ...
    SH
    chmod 0755, bin/"jiaoleaf-host-install-manifest"

    (bin/"jiaoleaf-host-uninstall-manifest").write <<~SH
      #!/usr/bin/env bash
      set -euo pipefail

      MANIFEST_PATH="$HOME/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.jiaoleaf.host.json"
      rm -f "$MANIFEST_PATH"
      echo "Removed native messaging manifest (if present): $MANIFEST_PATH"
    SH
    chmod 0755, bin/"jiaoleaf-host-uninstall-manifest"
  end

  def caveats
    <<~EOS
      To register the native messaging host manifest (Web Store extension ID by default):
        jiaoleaf-host-install-manifest

      Or, to register for a different (e.g. unpacked) extension ID:
        jiaoleaf-host-install-manifest <EXTENSION_ID>
