...
// In production we run compiled JS. In dev, full host builds can fail on
// unrelated type errors, so fall back to the local tsx runner for MCP servers.
function resolveStdioServerCommand(filename: string): StdioServerCommand {
  ...
  for (let i = 0; i < 10; i++) {
    if (fs.existsSync(path.join(dir, 'package.json'))) {
      const distPath = path.join(dir, 'dist', 'src', 'mcp', filename);
  ...
  const fallbackPath = path.resolve(thisDir, `../../mcp/${filename}`);
  return { command: 'node', args: [fallbackPath], displayPath: fallbackPath };
}
const mermaidStdioServer = resolveStdioServerCommand('mermaidStdioServer.js');
const askUserStdioServer = resolveStdioServerCommand('askUserStdioServer.js');
...
async function ensureMcpServersRegistered(
  ...
  const registrationKey = [
    ...
    ...askUserStdioServer.args,
  ].join('\0');
  if (mcpRegisteredForCli.has(registrationKey)) return;
  ...
  if (allSucceeded) mcpRegisteredForCli.add(registrationKey);
}
