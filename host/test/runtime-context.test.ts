...
test('GET /v1/runtime/claude/context returns context usage', async () => {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${address.port}/v1/runtime/claude/context`);
    ...
    assert.equal(body.usedTokens, 1200);
...
test('POST /v1/runtime/codex/context returns context usage', async () => {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${address.port}/v1/runtime/codex/context`, {
      method: 'POST',
